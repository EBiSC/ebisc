...
class NewBatchForm(forms.Form):
    ...
    batch_type = forms.CharField(label='New Batch Type', max_length=50, widget=forms.Select(choices=CelllineBatch.BATCH_TYPE_CHOICES), help_text=format_html(u'<div class="tooltip-item"><span class="glyphicon glyphicon-question-sign"></span><div class="tooltip"><p><b>Depositor expansion batch:</b> A batch-worth of empty vials are sent to the depositor, with EBiSC labels and EBiSC vial IDs. The depositor fills the vials and ships them back to central facility.</p><p><b>Central facility expansion batch:</b> Batches created for EBiSC1 in the Central facility.</p></div></div>'))
